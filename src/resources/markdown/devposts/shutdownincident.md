...
JetPortal Projects that you have saved to your computer were not directly affected by this outage.
Restore points in the JetPortal editor were also not affected.
...
This process did complete and on the 20th, assets for JetPortal projects were on Backblaze.
However, the process did strain a lot of the server's CPU (the server was very slow), and used a lot of network bandwidth.

*Technical detail:*
It also turned out that the majority of the server's space (over 60 gigabytes) was not actually used by JetPortal projects, but by Docker logs of what the server was doing.

## Server Outage
The next day, January 21st, we could no longer connect to the JetPortal projects server.

Ianyourgod (JetPortal server developer) could not connect to the live JetPortal server, could not login to our Oracle account, and the JetPortal server refused connections from any source.
...
**This section is only for transparency on what we tried, and not for harrassment.** Do not contact Oracle for anything related to JetPortal.
...
## Our Responsibility
JetPortal is not liable for any loss of data, but we still should have setup backups for your accounts & projects before anything like this could have happened.
